# Copy this file into <redmine_root>/config/initializers (any file name
# ending in .rb) and adjust the values below.
#
# The require is mandatory: Redmine runs plugin init.rb files in to_prepare,
# after the config initializers, while configure below must register the
# OmniAuth middleware before the middleware stack is built.
...
RedmineOmniauthSaml::Base.configure do |config|
  config.saml = {
    ...
    :idp_sso_target_url             => "http://sso.example.com/saml2/idp/SSOService.php", # SSO login endpoint
    :idp_cert_fingerprint           => "certificate fingerprint", # SSO ssl certificate fingerprint
    # Alternatively, specify the full certificate:
